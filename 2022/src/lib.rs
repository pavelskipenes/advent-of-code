...
#[must_use]
pub fn elf_carrying_most_calories(calorie_list: &str, top: usize) -> String {
    let mut calories_carried_per_elfs = calorie_list
        .split("\n\n")
        .map(|group| -> u64 {
            let lines = group.lines();

            let calories: Vec<u64> = lines
                .skip_while(|line| line.trim() == "")
                .map(|line| -> u64 { line.trim().parse::<u64>().unwrap_or(0) })
                .collect();
            let calorie_sum = calories.iter().sum::<u64>();
            calorie_sum
        })
        .collect::<Vec<u64>>();
...
mod tests {
    ...
    fn test_example() {
        ...
        const ANSWER0: &str = "24000";
        const ANSWER1: &str = "45000";

        let result = elf_carrying_most_calories(INPUT, 1);
        assert_eq!(result, ANSWER0);
        let result = elf_carrying_most_calories(INPUT, 3);
        assert_eq!(result, ANSWER1);
    }
    #[test]
    fn test_input() {
        let path = Path::new("puzzle_input/day_1/problem_1");
        let file_open_result = File::open(path);
        let mut file = file_open_result.unwrap();
        let mut input = String::new();
        // why does read_to_string() require a mut ref to file if it's only reading it?
